...
const categories = [
  ...
  { name: "Hair Salons", count: "842 businesses", icon: Sparkles, tone: "bg-purple-50 text-purple-500" },
  { name: "Plumbers", count: "567 businesses", icon: Wrench, tone: "bg-blue-50 text-blue-500" },
  ...
  return (
    <main className="min-h-screen bg-[#f7f7fa] font-[family-name:var(--font-geist-sans)] text-[#1c1d22]">
      ...
      <section className="px-5 py-12 md:px-6 md:py-[72px] lg:px-[max(30px,calc((100vw-1400px)/2))]" id="categories">
        <div className="mb-6 flex items-start justify-between gap-4 md:mb-9 md:items-end"><div><h2 className="mb-2.5 text-[25px] font-bold leading-none tracking-tight md:text-[29px] lg:text-[34px]">Popular Categories</h2><p className="text-[13px] text-[#5c6168] lg:text-[15px]">Browse the most searched industries in France</p></div><a className="flex shrink-0 items-center gap-2 whitespace-nowrap text-[11px] font-bold text-[#00663f] md:text-[13px] lg:text-sm" href="#categories">View all categories <ArrowRight size={17} /></a></div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6 lg:gap-5">
          {categories.map(({ name, count, icon: Icon, tone }) => <a href="#businesses" className="flex min-h-[145px] flex-col items-center rounded-2xl border border-[#eef0f1] bg-white px-3 pt-6 pb-4 shadow-[0_4px_8px_#10101005] lg:min-h-40" key={name}><span className={`mb-4 grid h-12 w-12 place-items-center rounded-xl ${tone}`}><Icon size={22} /></span><strong className="text-[13px] tracking-tight lg:text-[15px]">{name}</strong><small className="mt-2 text-[10px] text-[#676d73] lg:text-[11px]">{count}</small></a>)}
        </div>
      </section>
